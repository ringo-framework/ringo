  ...
  % for item in items[listing.pagination_start:listing.pagination_end]:
    ...
    % for num, field in enumerate(tableconfig.get_columns()):
      ...
      % endif
        ...
        <a class="link" title="${_('Open item in %s mode') % _(permission.capitalize())}" href="${request.route_path(h.get_action_routename(clazz, permission), id=item.id)}">${_(value) | h}</a>
        % endif
    </td>
    % endfor
  </tr>
