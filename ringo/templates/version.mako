...
<p>${h.literal(_('<a href="https://bitbucket.org/ti/ringo" target="_blank">Ringo</a> is a small Python based high level web application framework build with Pyramid . It provides basic functionality which is often used in modern web applications'))}</p>
<h3>Formbar ${formbar_version}</h3>
<p>${h.literal(_('<a href="https://bitbucket.org/ti/formbar" target="_blank">Formbar</a> is a Python library to layout, render and validate HTML forms in web applications.'))} </p>
<h3><img src="${request.static_path('ringo:static/images/sqla-logo.gif')}" alt="SQLAlchemy logo"/>SQLAlchemy ${sqlalchemy_version}</h3>
<p>${h.literal(_('<a href="www.sqlalchemy.org" target="_blank">SQLAlchemy</a> is Python based ORM mapper.'))}</p>
